...
import { Text, View, TouchableOpacity, StyleSheet, ScrollView, SafeAreaView, Image } from 'react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';
import Icon from 'react-native-vector-icons/FontAwesome';
import { useNavigation } from '@react-navigation/native';
import { setUser, logout } from '../actions/userActions'; // Redux action'ları
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const getUserData = async () => {
      ...
      if (storedUserName && token) {
        dispatch(setUser(storedUserName));  // Redux store'a kullanıcı bilgisini ekle
      } else {
        dispatch(logout());  // Giriş yapılmamışsa logout işlemi
      }
    };
  ...
  const handleLogout = async () => {
    await AsyncStorage.removeItem('userToken');
    await AsyncStorage.removeItem('username');
    dispatch(logout());  // Redux'ta da logout işlemi
  };
